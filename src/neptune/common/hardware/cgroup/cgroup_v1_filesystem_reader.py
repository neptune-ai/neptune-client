...
class CGroupV1FilesystemReader(CGroupAbstractFilesystemReader):
    ...
    def get_cpu_max_limits(self) -> tuple[Union[str, int], int]:
        ...
        if cpu_quota_micros == -1:
            return CGroupAbstractFilesystemReader.NO_LIMIT_VALUE, cpu_period_micros
        else:
            return cpu_quota_micros, cpu_period_micros

    def get_cpuacct_usage_nanos(self) -> int:
        return self.__read_int_file(self.__cpuacct_usage_file)

    def __get_cpu_quota_micros(self) -> int:
        return self.__read_int_file(self.__cpu_quota_file)

    def __get_cpu_period_micros(self) -> int:
