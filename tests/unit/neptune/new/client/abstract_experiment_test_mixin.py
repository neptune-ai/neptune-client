...
class AbstractExperimentTestMixin:
    ...
    def test_offline_mode(self):
        with self.call_init(mode="offline") as exp:
            ...
            exp_dirs = glob(f".neptune/offline/{exp.container_type.value}__{exp._id}__{os.getpid()}__*")
            self.assertTrue(len(exp_dirs) == 1)
            self.assertIn("data-1.log", os.listdir(exp_dirs[0]))
    ...
    def test_async_mode(self):
        with self.call_init(mode="async", flush_period=0.5) as exp:
            ...
            exp_dirs = glob(f".neptune/async/{exp.container_type.value}__{exp._id}__{os.getpid()}__*")
            self.assertTrue(len(exp_dirs) == 1)
            self.assertIn("data-1.log", os.listdir(exp_dirs[0]))
