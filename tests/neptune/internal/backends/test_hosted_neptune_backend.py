...
class TestHostedNeptuneBackend(unittest.TestCase):
    ...
    def test_convert_to_api_parameters(self, uuid4, swagger_client_factory):
        ...
        expected_api_params = {
            ApiParameter(
                id=some_uuid, name="str", parameterType="string", value="text"
            ),
            ApiParameter(
                id=some_uuid, name="bool", parameterType="string", value="False"
            ),
            ApiParameter(
                id=some_uuid, name="float", parameterType="double", value="1.23"
            ),
            ApiParameter(id=some_uuid, name="int", parameterType="double", value="12"),
            ApiParameter(
                id=some_uuid, name="inf", parameterType="string", value="Infinity"
            ),
            ApiParameter(
                id=some_uuid, name="-inf", parameterType="string", value="-Infinity"
            ),
            ...
            ApiParameter(
                id=some_uuid, name="obj", parameterType="string", value=str(some_object)
            ),
        }
    ...
    def test_delete_artifact(self, swagger_client_factory):
        ...
        # and
        def build_call(path):
            return call(experiment=experiment, path=path)

        # when
        leaderboard.delete_artifacts(
            experiment=experiment, path="/an_abs_path_in_exp_output"
        )
        leaderboard.delete_artifacts(
            experiment=experiment, path="/../an_abs_path_in_exp"
        )
        leaderboard.delete_artifacts(
            experiment=experiment, path="/../../an_abs_path_in_prj"
        )
        ...
        self.assertRaises(
            ...
            path="test/../../a_path_outside_exp",
        )
        self.assertRaises(
            ValueError,
            leaderboard.delete_artifacts,
            experiment=experiment,
            path="../a_path_outside_exp",
        )
        self.assertRaises(
            ValueError, leaderboard.delete_artifacts, experiment=experiment, path=".."
        )
