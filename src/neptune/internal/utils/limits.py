...
def image_size_exceeds_limit_for_logging(content_size: int) -> bool:
    if content_size > LOGGED_IMAGE_SIZE_LIMIT_MB * BYTES_IN_MB:
        warnings.warn(
            f"You are attempting to log an image that is {content_size / BYTES_IN_MB:.2f}MB large. "
            f"Neptune supports logging images smaller than {LOGGED_IMAGE_SIZE_LIMIT_MB}MB. "
