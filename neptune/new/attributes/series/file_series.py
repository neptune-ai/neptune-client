...
class FileSeries(Series[Val, Data]):
    ...
    def download(self, destination: Optional[str]):
        ...
        item_count = self._backend.get_image_series_values(self._experiment_uuid, self._path, 0, 1).totalItemCount
        for i in range(0, item_count):
            self._backend.download_file_series_by_index(self._experiment_uuid, self._path, i, target_dir)

    def download_last(self, destination: Optional[str]):
        target_dir = self._get_destination(destination)
        item_count = self._backend.get_image_series_values(self._experiment_uuid, self._path, 0, 1).totalItemCount
        if item_count > 0:
            self._backend.download_file_series_by_index(self._experiment_uuid, self._path, item_count - 1, target_dir)
        else:
