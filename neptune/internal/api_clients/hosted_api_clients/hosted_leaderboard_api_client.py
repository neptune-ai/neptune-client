...
from neptune.internal.utils.http_utils import (
...
class HostedNeptuneLeaderboardApiClient(HostedNeptuneMixin, LeaderboardApiClient):
    ...
    def get_leaderboard_entries(
        ...
        try:
            ...
            return [
                LeaderboardEntry(e) for e in self._get_all_items(get_portion, step=100)
            ]
        except HTTPNotFound:
    ...
    def websockets_factory(self, project_id, experiment_id):
        ...
        return ReconnectingWebsocketFactory(
            backend=self, url=base_url + "/experiments/" + experiment_id + "/operations"
        )
    ...
    def create_experiment(
        ...
        try:
            ...
            api_experiment = (
                self.backend_swagger_client.api.createExperiment(**kwargs)
                .response()
                .result
            )
            ...
            if (
                extract_response_field(e.response, "type")
                == "LIMIT_OF_EXPERIMENTS_IN_PROJECT_REACHED"
            ):
    ...
    def get_experiment(self, experiment_id):
        return (
            self.backend_swagger_client.api.getExperiment(experimentId=experiment_id)
            .response()
            .result
        )
    ...
    def update_experiment(self, experiment, properties):
        EditExperimentParams = self.backend_swagger_client.get_model(
        ...
        try:
            self.backend_swagger_client.api.updateExperiment(
                ...
                editExperimentParams=EditExperimentParams(
                    properties=[
                        KeyValueProperty(key=key, value=properties[key])
    ...
    def set_property(self, experiment, key, value):
        properties = {
            p.key: p.value
            for p in self.get_experiment(experiment.internal_id).properties
        }
        properties[key] = str(value)
        return self.update_experiment(experiment=experiment, properties=properties)
    ...
    def create_channel(self, experiment, name, channel_type) -> ChannelWithLastValue:
        ...
        try:
            params = ChannelParams(name=name, channelType=channel_type)

            channel = (
                self.backend_swagger_client.api.createChannel(
                    experimentId=experiment.internal_id, channelToCreate=params
                )
                .response()
                .result
            )
    ...
    def get_channels(self, experiment) -> Dict[str, object]:
        ...
        channels_last_values_by_name = dict(
            (ch.channelName, ch) for ch in api_experiment.channelsLastValues
        )
        channels = dict()
    ...
    def reset_channel(self, experiment, channel_id, channel_name, channel_type):
        try:
            self.backend_swagger_client.api.resetChannel(id=channel_id).response()
        except HTTPNotFound:
            # pylint: disable=protected-access
    ...
    def create_system_channel(
        ...
        try:
            ...
            channel = (
                self.backend_swagger_client.api.createSystemChannel(
                    experimentId=experiment.internal_id, channelToCreate=params
                )
                .response()
                .result
    ...
    def get_system_channels(self, experiment) -> Dict[str, object]:
        try:
            channels = (
                self.backend_swagger_client.api.getSystemChannels(
                    experimentId=experiment.internal_id,
                )
                .response()
                .result
        ...
        for channel_with_values in channels_with_values:
            points = [
                Point(
                    ...
                    x=value.x,
                    y=Y(
                        numericValue=value.y.get("numeric_value"),
            ...
            input_channels_values.append(
                InputChannelValues(
                    channelId=channel_with_values.channel_id, values=points
                )
            )

        try:
            batch_errors = (
                self.backend_swagger_client.api.postChannelValues(
                    experimentId=experiment.internal_id,
    ...
    def mark_succeeded(self, experiment):
        CompletedExperimentParams = self.backend_swagger_client.get_model(
        ...
        try:
            self.backend_swagger_client.api.markExperimentCompleted(
                ...
                completedExperimentParams=CompletedExperimentParams(
                    state="succeeded", traceback=""  # FIXME
                ),
            ).response()
    ...
    def mark_failed(self, experiment, traceback):
        ...
        try:
            self.backend_swagger_client.api.markExperimentCompleted(
                experimentId=experiment.internal_id,
                completedExperimentParams=CompletedExperimentParams(
                    state="failed", traceback=traceback
                ),
            ).response()
    ...
    def ping_experiment(self, experiment):
        try:
            self.backend_swagger_client.api.pingExperiment(
                experimentId=experiment.internal_id
            ).response()
        except HTTPNotFound:
            # pylint: disable=protected-access
    ...
    def create_hardware_metric(self, experiment, metric):
        ...
        try:
            ...
            metric_dto = (
                self.backend_swagger_client.api.createSystemMetric(
                    experimentId=experiment.internal_id,
                    metricToCreate=system_metric_params,
                )
                .response()
                .result
            )
    ...
    def send_hardware_metric_reports(self, experiment, metrics, metric_reports):
        ...
        try:
            ...
            response = self.backend_swagger_client.api.postSystemMetricValues(
                experimentId=experiment.internal_id, metricValues=system_metric_values
            ).response()
        ...
        if isinstance(artifact, str):
            if os.path.exists(artifact):
                ...
                upload_entry = UploadEntry(
                    os.path.abspath(artifact), normalize_file_name(target_name)
                )
            else:
    ...
    def rm_data(self, experiment, path):
        try:
            return (
                self.backend_swagger_client.api.deleteExperimentOutput(
                    experimentIdentifier=str(experiment.internal_id), path=path
                )
                .response()
                .result
            )
    ...
    def download_data(self, experiment, path, destination):
        project_storage_path = "/{exp_id}/output/{file}".format(
            exp_id=experiment.id, file=path
        )
        ...
        with self._download_raw_data(
            ...
            if response.status_code == NOT_FOUND:
                raise PathInProjectNotFound(
                    path=path, project_identifier=project.full_id
                )
            else:
                response.raise_for_status()
    ...
    def prepare_source_download_request(self, experiment, path):
        try:
            return (
                self.backend_swagger_client.api.prepareForDownload(
    ...
    def get_download_request(self, request_id):
        return (
            self.backend_swagger_client.api.getDownloadRequest(id=request_id)
            .response()
            .result
        )
    ...
    def _convert_to_api_properties(self, raw_properties):
        ...
        return [
            KeyValueProperty(key=key, value=str(value))
    ...
    def _convert_channel_to_channel_with_last_value(self, channel):
        ChannelWithValueDTO = self.leaderboard_swagger_client.get_model(
            "ChannelWithValueDTO"
        )
    ...
    def _upload_loop(self, fun, data, progress_indicator, **kwargs):
        ...
        for part in data.generate():
            ret = with_api_exceptions_handler(self._upload_loop_chunk)(
                fun, part, data, **kwargs
            )
